...
export default async function CanvasDashboard() {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();

  // Fetch Folders AND Canvases (exclude text notes)
  const { data: docs } = await supabase
    .from('documents')
    .select('id, title, is_folder, parent_id, type, updated_at')
    .eq('user_id', user?.id)
    .or('type.eq.folder,type.eq.canvas') // <--- The Filter
    ...
    <main className="min-h-screen bg-[#050505] p-6 md:p-12">
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Anatomy & Sketching</p>
        </div>
      </header>

      {/* Reuse the File Manager */}
      <FileManager initialDocs={docs || []} mode="canvas" />
    </main>
  );
}